  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
          ...
          ? `<ul class="participants-list">
              ${details.participants.map(participant => `
                <li>
                  ...
                  <span class="delete-participant" title="Remover" data-activity="${name}" data-participant="${participant}">&#128465;</span>
                </li>
              `).join('')}
            </ul>`
        ...
        // Adiciona event listener para ícones de deletar
        ...
        deleteIcons.forEach(icon => {
          icon.addEventListener('click', async (e) => {
            ...
            if (confirm(`Remover ${participantEmail} de ${activityName}?`)) {
              try {
